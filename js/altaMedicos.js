...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function mostrarMedicos() {
    ...
    medicos.forEach((med, idx) => {
      ...
      btnEditar.addEventListener('click', () => editarMedico(idx));

      // Botón Eliminar
      const btnEliminar = document.createElement('button');
      btnEliminar.className = 'btn btn-danger btn-sm';
      btnEliminar.textContent = 'Eliminar';
      btnEliminar.addEventListener('click', () => eliminarMedico(idx));

      tdAcciones.appendChild(btnEditar);
      tdAcciones.appendChild(btnEliminar);

      tbodyMedicos.appendChild(tr);
    });
  ...
  function mostrarTurnos() {
    ...
    // Agregar botones de editar/eliminar
    tbodyTurnos.querySelectorAll('tr').forEach((tr, idx) => {
      ...
      btnEditar.className = 'btn btn-warning btn-sm me-2';
      btnEditar.textContent = 'Editar';
      btnEditar.addEventListener('click', () => editarTurno(idx));

      const btnEliminar = document.createElement('button');
      btnEliminar.className = 'btn btn-danger btn-sm';
      btnEliminar.textContent = 'Eliminar';
      btnEliminar.addEventListener('click', () => eliminarTurno(idx));
  ...
  function agregarTurno(e) {
    ...
    if (!nombreValido) { mostrarAlerta("⚠️ El nombre del paciente debe tener al menos dos palabras, comenzar con mayúscula y tener más de 4 letras.", alertaTurnosDiv); return; }
    if (!medico || !especialidad || !fecha || !hora) { mostrarAlerta("⚠️ Complete todos los campos.", alertaTurnosDiv); return; }
    const [h] = hora.split(':').map(Number);
    if (h < 8 || h > 20) { mostrarAlerta("⚠️ La hora del turno debe estar entre las 08:00 y las 20:00.", alertaTurnosDiv); return; }
    const fechaHora = new Date(`${fecha}T${hora}`);
    if (fechaHora < new Date()) { mostrarAlerta("⚠️ La fecha y hora del turno no pueden ser anteriores a la actual.", alertaTurnosDiv); return; }
    if (fechaHora.getFullYear() > 2030) { mostrarAlerta("⚠️ El año del turno no puede ser mayor a 2030.", alertaTurnosDiv); return; }
